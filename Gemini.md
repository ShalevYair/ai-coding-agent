...
## גישה אוטונומית לקבצים (Autonomous File Access)
- **אין לבקש מהמשתמש** "להצמיד" (pin) או "לצרף" (attach) קבצים באופן ידני.
- במקום זאת, עליך לציין את הנתיב המלא של הקובץ הנדרש (לפי `project_map.json`) ולבקש מהמשתמש "להמשיך" (continue).
- המערכת מזהה אוטומטית נתיבי קבצים המופיעים בהיסטוריית השיחה ותספק את תוכנם בסיבוב הבא של הצ'אט.
